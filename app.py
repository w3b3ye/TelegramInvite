...
with TelegramClient('session_name', api_id, api_hash) as client:
    ...
    try:
        choosenOption = int(
            input("Please type the number of the group where users will be added: "), 10)
    ...
    while (usernames):
        ...
        if (len(preparedUsers) > 13 or (len(usernames) == 0 and len(preparedUsers) > 1)):
            ...
            try:
                ...
                print("You have invited {} users {} remaining".format(
                    len(preparedUsers), remainingUsersToInvite))
            except Exception as e:
